...
const BusinessPageHero = ({ text1, bluetext, text3, heroStar, description, signupbtns, gridimage, showbinder, className }) => {
    return (
        <div className={`mt-[100px] flex flex-col md:flex-row items-center gap-20 justify-center md:justify-between ${showbinder ? 'pt-32 pb-20' : 'py-20'}`}>
            ...
            <div className={`relative max-w-[580px] w-[100%]`}><img src={gridimage} alt="" className="m-auto"/>{showbinder ? <img src={dashboardBind} className="absolute top-[22%] left-[-20px]" alt="" /> : null}</div>
        </div>
    )
}
